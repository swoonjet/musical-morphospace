...
### 1. System name
Ascending Pythagorean Arc

### 2. Pitch organization
Pythagorean tuning (ratios from stacked 3/2 fifths reduced into an octave): 1, 9/8, 81/64, 4/3, 3/2, 27/16, 243/128, 2. Octave-repeating. Two octaves are available (indices 0-13). The scale is **adaptive_contextual**: each section of the piece uses a different subset.

- Slow section: pentatonic {0, 1, 2, 4, 5}
- Walk: pentatonic + P4 (index 3)
- Flights: full 7-note scale
- Peak: full + upper octave
- Sink: contracts back to just {0, 1}

This is a system where the *available pitch set is itself a formal parameter*.

### 3. Rhythmic organization
Additive 4+3 at 250 ms unit. Rhythmic ornament only: attacks may be displaced within the cycle; note durations vary freely (0.3-1.5 units). The slow voice improvises phrase-by-phrase; metric coherence is maintained at the cycle level, not the note level.

### 4. Formal structure
Arc-buildup, monotonic rise from minimum (single held drone, silence-heavy) to maximum density at the peak, then decisive fall back to silence on home. The *pitch set* expands and contracts with the arc — the scale itself is the dynamic shape.

### 5. Texture and voicing
Layered ostinato over continuous drone; two antiphonal choirs (left, right) alternate held chords as the harmonic ground; a solo voice and counter-flute improvise over it. Voice independence is very low — when choirs sing, they sing as bodies.

### 6. Ornament and inflection
Rhythmic-only. Implemented as timing variation and improvisational note-length choice in the melodic voices. No melodic ornament, no pitch bends.

### 7. Performance context
Ritual-religious. Listener is a ritual participant. Transmission is notated-prescriptive for the harmonic ground (the chord exchanges); the melodic voices improvise within the scale subset of each section. Silences at transitions are structural — the scale subset changes during silence.

### 8. Relationship to neighboring systems
Shares with **Hindustani khayal** the drone-based tonal hierarchy, slow-to-fast arc (alap → jor → jhala), and improvised melodic elaboration over a held ground. Shares with **Arabic maqam** the modal character and the section-by-section shift of the pitch set. Shares with **Carnatic RTP** the improvised solo over layered accompaniment. Diverges by using strict Pythagorean ratios (rare outside Western medieval theory) and by making the *adaptive scale subset* itself a formal device rather than a performer's choice. The unique synthesis: *Pythagorean-tuned layered-ostinato arc where the scale subset expands and contracts with the dramatic intensity*.
...
### 10. Field note
The drone is the only pitch heard from first second to last. Everything else happens on top of it, beside it, pulling it slightly away — and returning.
